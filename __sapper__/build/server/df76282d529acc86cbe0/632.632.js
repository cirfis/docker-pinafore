...
var store = __webpack_require__(692);
...
var requestPostAnimationFrame = __webpack_require__(7291);
...
class RealmStore extends store/* Store */.y {
  constructor (init, maxSize) {
    super(init)
    this.set({ realms: new quick_lru/* QuickLRU */.c({ maxSize: maxSize }) })
    this._batches = {}
  }
...
function reselect (store, outputKey, inputKey) {
  ...
  store.compute(countKey, [inputKey], input => {
    ...
    if (!(0,isEqual/* default */.Z)(prevValue, input)) {
      nextValue = input
      count++
    }
    return count
